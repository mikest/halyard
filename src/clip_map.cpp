...
int ClipMap::flags_for_position(const Vector2 &pos) {
    int flags = 0;
    if (pos.x > 0.5f) flags |= SEAM_RIGHT;
    if (pos.x < -0.5f) flags |= SEAM_LEFT;
    if (pos.y > 0.5f) flags |= SEAM_TOP;
    if (pos.y < -0.5f) flags |= SEAM_BOTTOM;
    return flags;
}
...
void ClipMapInstance::set_tile_size(int p_size) {
    ...
    if (p_size % 2 != 0) p_size += 1;  // Must be even for seams
